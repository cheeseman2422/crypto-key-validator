...
interface CryptoValidatorAPI {
  ...
  exportResults: (filePath: string, format: string, options: any) => Promise<{ success: boolean }>;
...
const api: CryptoValidatorAPI = {
  ...
  // Export functionality
  exportResults: (filePath: string, format: string, options: any) => ipcRenderer.invoke('export-results', filePath, format, options),
